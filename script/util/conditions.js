export class conditions {
  ...
  static async onActionCondition(data) {
    const condition = await CONFIG.statusEffects.find(
      (it) => it.id == data.property
    );
    if (!condition) return;

    const token = await Array.from(game.scenes.current.tokens).find(
      (it) => it._id == data.combatant.tokenId
    );
    const tokenCondition = token.actor.effects.find(
      (it) => it.icon == condition.icon
    );
    ...
    const slowCondition = await CONFIG.statusEffects.find(
      (it) => it.id == "slowAction"
    );

    const slowAction = await token.actor.effects.find(
      (it) => it.icon == slowCondition.icon
    );
    const fastAction = await token.actor.effects.find(
