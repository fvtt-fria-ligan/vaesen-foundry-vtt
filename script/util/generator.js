...
export class generator {
  ...
  static async generateCharacter(actor) {
    ...
    const upbringingKeys = Object.keys(
      generator_data.changeList[classSelected]
    );
    ...
    const professionKeys = Object.keys(
      generator_data.professionTable[classSelected]
    );
    const professionRoll = await this._getRoll(professionKeys.length);
    const professionName =
      generator_data.professionTable[classSelected][professionRoll];
    const professionSelected = generator_data.professionsList[professionName];
    const archetytpePageLink = game.journal
      .getName("Chapter 2 - Your Player Character")
      ?.pages.getName(professionSelected.archetype)?.uuid;
    ...
    const ageInfo = generator_data.ageInfo.find(
      (it) => it.min <= age && it.max >= age
    );
    // console.log("Vaesen | Generator | Age Info", age, ageInfo);

    const archetypeInfo =
      generator_data.archetypeInfo[professionSelected.archetype];
    ...
    if (
      itemsToCreate.filter((it) => it.name.toLowerCase() == "crowbar").length ==
      1
    ) {
      itemsToCreate.push(
        toStartingObject(getItemInfo("Crowbar - Gear", "gear"))
      );
    }
    ...
    for (let index = 0; index < ageInfo.reducedAttribute; index++) {
      ...
      changes[
        `system.attribute.${generator_data.attributeList[attributeReducedRoll]}.value`
      ] -= 1;
      attributeReduced = attributeReducedRoll;
    }

    changes[
      "system.bio.archetype"
    ] = `${professionSelected.archetype} (${professionName})`;
    ...
    <ul>
        <li><b>${game.i18n.localize(
          "GENERATOR.CLASS"
        )}:</b> ${classSelected}</li>
        <li><b>${game.i18n.localize(
          "GENERATOR.UPBRINGING"
        )}:</b> ${upbringingSelected}</li>
        <li><b>${game.i18n.localize(
          "BIO.ARCHETYPE"
        )}:</b> @UUID[${archetytpePageLink}]{${
      professionSelected.archetype
    }}</li>
        <li><b>${game.i18n.localize(
          "GENERATOR.PROFESSION"
        )}:</b> ${professionName}</li>
        <li><b>${game.i18n.localize("RESOURCES")}:</b> ${resources}</li>
        <li><b>${game.i18n.localize("BIO.AGE")}:</b> ${age} (${
      ageInfo.name
    })</li>
        ...
        <li><b>${game.i18n
          .localize("HEADER.TALENTS")
          .toLowerCase()
          .replace(/\b(\w)/g, (x) => x.toUpperCase())}:</b> @UUID[Item.${
      talentItem._id
    }]{${talent}}</li>
    ...
    dialogHtml.push(
      `<div class="info-box flex row fancy-border"><img class="icon pi-1" src="systems/vaesen/asset/hazard-sign.svg" width=64> <p>${game.i18n.localize(
        "GENERATOR.MESSAGE"
      )}</p></div>`
    );
    ...
    dialogHtml.push(`
        ...
        <b class="title">${game.i18n.localize(
          "HEADER.ATTRIBUTES"
        )} & ${game.i18n.localize("HEADER.SKILLS")}</b>
        <div class="flex row wrap">
        <div class="skills flex column physique">
        <b class="header"> ${game.i18n.localize("ATTRIBUTE.PHYSIQUE")}: ${
      changes["system.attribute.physique.value"]
    }</b>
          <div class="skill-group">
            <div class="skill flex row align-center"><b>${game.i18n.localize(
              "SKILL.AGILITY"
            )}:</b><p class="pi-3">${
      changes["system.skill.agility.value"]
    }</p></div>
            <div class="skill flex row align-center"><b>${game.i18n.localize(
              "SKILL.CLOSE_COMBAT"
            )}:</b> <p class="pi-3">${
      changes["system.skill.closeCombat.value"]
    }</p></div>
            <div class="skill flex row align-center"><b>${game.i18n.localize(
              "SKILL.FORCE"
            )}:</b> <p class="pi-3">${
      changes["system.skill.force.value"]
    }</p></div>
          </div>
        </div>
        
        <div class="skills flex column logic">
        <b class="header"> ${game.i18n.localize("ATTRIBUTE.LOGIC")}: ${
      changes["system.attribute.logic.value"]
    }</b>
          <div class="skill-group">
            <div class="skill flex row align-center"><b>${game.i18n.localize(
              "SKILL.INVESTIGATION"
            )}:</b><p class="pi-3">${
      changes["system.skill.investigation.value"]
    }</p></div>
            <div class="skill flex row align-center"><b>${game.i18n.localize(
              "SKILL.LEARNING"
            )}:</b> <p class="pi-3">${
      changes["system.skill.learning.value"]
    }</p></div>
            <div class="skill flex row align-center"><b>${game.i18n.localize(
              "SKILL.VIGILANCE"
            )}:</b> <p class="pi-3">${
      changes["system.skill.vigilance.value"]
    }</p></div>
          </div>
        </div>
       </div>
       ...
       <div class="skills flex column precision">
        <b class="header"> ${game.i18n.localize("ATTRIBUTE.PRECISION")}: ${
      changes["system.attribute.precision.value"]
    }</b>
          <div class="skill-group">
            <div class="skill flex row align-center"><b>${game.i18n.localize(
              "SKILL.MEDICINE"
            )}:</b><p class="pi-3">${
      changes["system.skill.medicine.value"]
    }</p></div>
            <div class="skill flex row align-center"><b>${game.i18n.localize(
              "SKILL.RANGED_COMBAT"
            )}:</b> <p class="pi-3">${
      changes["system.skill.rangedCombat.value"]
    }</p></div>
            <div class="skill flex row align-center"><b>${game.i18n.localize(
              "SKILL.STEALTH"
            )}:</b> <p class="pi-3">${
      changes["system.skill.stealth.value"]
    }</p></div>
          </div>
        </div>

        <div class="skills flex column empathy">
        <b class="header"> ${game.i18n.localize("ATTRIBUTE.EMPATHY")}: ${
      changes["system.attribute.empathy.value"]
    }</b>
          <div class="skill-group">
            <div class="skill flex row align-center"><b>${game.i18n.localize(
              "SKILL.INSPIRATION"
            )}:</b><p class="pi-3">${
      changes["system.skill.inspiration.value"]
    }</p></div>
            <div class="skill flex row align-center"><b>${game.i18n.localize(
              "SKILL.MANIPULATION"
            )}:</b> <p class="pi-3">${
      changes["system.skill.manipulation.value"]
    }</p></div>
            <div class="skill flex row align-center"><b>${game.i18n.localize(
              "SKILL.OBSERVATION"
            )}:</b> <p class="pi-3">${
      changes["system.skill.observation.value"]
    }</p></div>
          </div>
        </div>
...
function getItemInfo(itemName, type) {
  ...
  if (item == undefined) {
    let itemType = generator_data.weaponList.includes(itemName.toLowerCase())
      ? "weapon"
      : type;
    item = { type: itemType, name: itemName };
  } else item = item.toObject();
